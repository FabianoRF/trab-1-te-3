# Aula Prática 3 - Mineração de Regra de Associação 

 
# Mesmo que tenha mais de 20 anos, o Market Basket Analysis (MBA) (ou Mineração de Regras de Associação) 
...
# Nesta aula, vamos buscar a associação entre os clubes de futebol da Europa e responder a pergunta:
...
# Preparando os dados para mineração das regras de associção
...
#### UFA... Acabou ####
# Estamos apenas começando, vamos ao Trabalho Prático I
...
# O que fazer?
# Neste trabalho vamos explorar os recursos para análise de MBA em R, para isso
# vamos utilizar um banco de dados e analisar suas possíveis relações (MBA). A sua
# tarefa é:
# 1- Buscar um banco de dados no repositório Kaggle (https://www.kaggle.com) que
# permita a análise de dados por meio de MBA
# 2- Realizar o pré-processamento para esses dados
# 3- Realizar a análise desses dados, buscando insights e informações que possivelmente
# estavam "escondidas"
# 4- Utilizar recursos para visualização a partir de redes e tabelas interativas

# O que entregar?
# Você deverá entregar dois arquivos:
# 1- Script em R (como esse apresentado na aula prática)
# 2- Relatório (em pdf) com toda a descrição das tarefas realizadas. O seu relatório
# deve apresentar todas as funcões utilizadas e uma explicação sobre sua utilização.
# Apresente no relatório trechos da execução (Views, Tabelas, Gráficos, etc). Seu relatório
# deve ter entre 3 e 7 páginas.
# ** Não é necessário que todos os componentes enviem, somente um (coloque no cabeçalho o nome de todos)
# Cuidado, toda avaliação será em cima do script e do relatório, não teremos apresentações.
